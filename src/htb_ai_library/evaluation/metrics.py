"""
Metrics for evaluating attack effectiveness and model confidence.
"""
...
def evaluate_attack_effectiveness(
    ...
    with torch.no_grad():
        ...
        clean_correct_total = clean_correct.float().sum().item()
        flipped_total = flipped.float().sum().item()
        attack_success_rate = flipped_total / clean_correct_total if clean_correct_total > 0 else 0.0
...
def print_attack_summary(results: Dict[str, float]) -> None:
    """
    Print a formatted summary of attack evaluation results.
    """
