"""
Comprehensive visualization suite for analyzing adversarial attacks.
"""
...
def _style_axes(ax: plt.Axes):
    """Apply Hack The Box dark theme to a matplotlib Axes object."""
...
def visualize_attack(
    ...
    """
    Generic visualization for comparing a clean image and its adversarial version.
    """
    model.eval()
...
def plot_attack_effectiveness(attack_results):
    """
    Plots a sigmoid-like curve for attack effectiveness vs. effort.
    Assumes `attack_results` is a DataFrame with 'num_words' and 'evasion_rate'.
    """
...
def visualize_perturbation_analysis(results):
    """
    Visualizes perturbation characteristics from a list of attack results.
    """
    if not results:
